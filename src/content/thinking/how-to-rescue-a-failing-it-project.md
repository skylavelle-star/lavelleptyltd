...
The hard conversation that must happen early is with the program sponsor: the current plan is not achievable, so continuing to report against it is making the situation worse. This conversation is difficult, but it is necessary before any other recovery work can proceed.
...
The output of this assessment is a written finding. Not a set of slides, not a verbal summary, but a written document that sets out the actual state of the program, the causes of the current situation and what recovery will require. This document becomes the basis for every subsequent conversation.
...
**Rescope and replan.** Reduce the program to what can actually be delivered within the remaining budget and a credible timeframe. This requires decisions about what is in scope and what is not, so those decisions need to be made by the sponsor, not the delivery team.
...
**Define what success looks like now.** The original success criteria for the program may no longer be achievable. Redefining success is not the same as accepting failure, but is being clear about what the program can now deliver and why that still has value.
...
The Lavelle Pty Ltd Project Recovery Pack includes structured templates for health assessment, stakeholder reset communications, recovery roadmap planning and risk re-baselining. See [PMO Resources](/pmo-resources#project-recovery-pack) for details.
